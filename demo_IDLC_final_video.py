...
from core_IDLC.model_lm_talking_tran import build_model_idlc
# from core_IDLC.checkpoint import build_model_idlc
...
class Solver_2(nn.Module):
    def __init__(self, args):
        ...
        _, self.nets_ema = build_model_idlc(args)
    ...
    def sample(self, src_rgb, ref_lm):
        ...
        # print(fake_lm[0].unsqueeze(0).shape)
        # print(fake_lm.shape)
...
cap = cv2.VideoCapture('D:/database/dev/mp4/id00024/3ysHAigPe68/00001.mp4')
...
out = cv2.VideoWriter('output.avi', fourcc, 5, (1024, 1024))
...
def drawshape(landmarks):
    ...
    if diff<=30 and int(float(landmarks[30][0])) > int(float(landmarks[42][0])):
        ...
        for n in range(57, 59):
            cv2.line(img, (int(float(landmarks[n][0])), int(float(landmarks[n][1]))),
                     (int(floact(landmarks[n + 1][0])), int(float(landmarks[n + 1][1]))), line_color, line_width)
...
img_crop = cv2.imread('./for_demo2/0005000.jpg')
img_crop_2 = cv2.imread('./for_demo2/0002525.jpg')
img_crop_3 = cv2.imread('./for_demo2/0000425.jpg')
img_crop_4 = cv2.imread('./for_demo2/0000375.jpg')

# img_crop = cv2.imread('./for_demo2/0003075.jpg')
# img_crop_2 = cv2.imread('./for_demo2/0002625.jpg')
# img_crop_3 = cv2.imread('./for_demo2/0000425.jpg')
# img_crop_4 = cv2.imread('./for_demo2/0002675.jpg')
